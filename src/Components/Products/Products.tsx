...
const Products = () => {
    ...
    const submitAddHandler = (product: { name: string, filter: string }) => {
        ...
        addToast(`${product.name} successfuly added to ${product.filter}'s`, {appearance: "success"});
        setIsShow(false);
    }
    ...
    return (
        <section className="p-3 mt-10 shadow-inner rounded-xl bg-white">
            ...
            {(products.length === 0 || isShow) && <ProductForm id={edit} handleAdd={submitAddHandler} handleEdit={submitEditHandler} />}
            {products.length > 0 && <ShowButton product show={isShow} setShow={showHandler} />}
        </section>
    )
}
